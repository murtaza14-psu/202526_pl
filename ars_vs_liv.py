...
def clean_data(data):
    cleaned_data = data[['homeTeam.name', 'awayTeam.name', 'score.fullTime.home', 'score.fullTime.away', 'matchday']].copy()
    ...
    # extract match date
    cleaned_data['match_date'] = data['utcDate'][:10] if pd.notnull(data['utcDate']) else None
    cleaned_data["standings_date"] = pd.to_datetime(cleaned_data["match_date"]) - pd.Timedelta(days=1)
...
#function to get home and away team position in the standings table
def get_positions_from_csv(df, home_team, away_team, season, matchday):
    """
    Returns the league positions of the home and away teams from the standings 
    DataFrame for a given season and matchday.
    Returns (home_position, away_position) or (None, None) if not found.
    """
    standings = df[(df['season'] == season) & (df['matchday'] == matchday)]
    home_row = standings[standings['team'] == home_team]
    away_row = standings[standings['team'] == away_team]
    home_pos = int(home_row['position'].values[0]) if not home_row.empty else None
    away_pos = int(away_row['position'].values[0]) if not away_row.empty else None
...
for idx, row in matches.iterrows():
    ...
    #standings features
    standings_df = pd.read_csv("standings_2023_2025.csv")
    home_position, away_position = get_positions_from_csv(standings_df, home_team, away_team, season, matchday) 
    ...
    features.append({
        **row.to_dict(),
        ...
        #position feature
        'home_team_position': home_position,
        'away_team_position': away_position,
...
X_train_unshuffled, X_test_unshuffled, y_train_unshuffled, y_test_unshuffled = train_test_split(X, y, test_size=0.25, random_state=42, shuffle=False)
...
# Predicting ARS vs LIV match 
ars_form, ars_goals_for, ars_goals_against = get_team_last5_stats(matches, len(matches)-1, 'Arsenal FC', 2025)
liv_form, liv_goals_for, liv_goals_against = get_team_last5_stats(matches, len(matches)-1, 'Liverpool FC', 2025 )

difference = ars_form - liv_form
h2h_ars_goals_scored, h2h_ars_goals_conceded, h2h_liv_goals_scored, h2h_liv_goals_conceded = get_h2h_last3_stats(matches, len(matches)-1, 'Arsenal FC', 'Liverpool FC')
h2h_goals_scored_diff_match = h2h_ars_goals_scored - h2h_liv_goals_scored
h2h_goals_conceded_diff_match =  h2h_ars_goals_conceded - h2h_liv_goals_conceded

#positional
ars_pos, liv_pos = get_positions_from_csv(standings_df, 'Arsenal FC', 'Liverpool FC', 2025, 21)
positional_diff = ars_pos - liv_pos
X_new = pd.DataFrame(
    [[difference, ars_goals_for, liv_goals_for, ars_goals_against, liv_goals_against, h2h_goals_scored_diff_match, h2h_goals_conceded_diff_match, positional_diff]],
...
if predicted_outcome == 0:
    ...
    outcome = "Draw"
print("\n\n The predicted outcome for Arsenal vs Liverpool is:", outcome)

# Predict probabilities
probs = model.predict_proba(X_new)
print("\n\n Predicted probabilities for match outcomes (Arsenal, Liverpool, Draw):", probs[0])
